...
public:
  calltable();
  int add(char *call_id, unsigned long call_id_len, time_t time);
  int find_by_call_id(char *call_id, unsigned long call_id_len);
  int add_ip_port(int call_idx, in_addr_t addr, unsigned short port);
  int find_ip_port(in_addr_t addr, unsigned short port);
  int find_ip_port_ssrc(in_addr_t addr, unsigned short port, uint32_t ssrc,
                        int *idx_leg, int *idx_rtp);
