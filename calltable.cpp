...
calltable::calltable() {
  table_size = 0;
  table =
      (calltable_element *)malloc(sizeof(calltable_element) * calltable_max);
  erase_non_t38 = 0;
}

int calltable::add(char *call_id, unsigned long call_id_len, time_t time) {
...
int calltable::find_by_call_id(char *call_id, unsigned long call_id_len) {
  int i;
  for (i = 0; i < (int)table_size; i++) {
    if ((table[i].is_used != 0) && (table[i].call_id_len == call_id_len) &&
...
int calltable::add_ip_port(int call_idx, in_addr_t addr, unsigned short port) {
  int i, found;
  ...
  found = 0;
  for (i = 0; i < n; i++) {
    if (table[call_idx].ip[i] == addr && table[call_idx].port[i] == port) {
      found = 1;
      break;
    }
  }
...
int calltable::find_ip_port(in_addr_t addr, unsigned short port) {
  int idx, i;
...
int calltable::find_ip_port_ssrc(in_addr_t addr, unsigned short port,
                                 uint32_t ssrc, int *idx_leg, int *idx_rtp) {
  int i_leg, i_rtp;
