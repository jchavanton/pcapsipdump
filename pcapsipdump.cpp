...
int get_sip_peername(char *data, int data_len, const char *tag, char *caller,
                     int caller_len);
int get_ip_port_from_sdp(char *sdp_text, in_addr_t *addr, unsigned short *port);
char *gettag(const void *ptr, unsigned long len, const char *tag,
             unsigned long *gettaglen);
...
int main(int argc, char *argv[]) {
  ...
  // allow interface to be specified without '-i' option - for sake of
  // compatibility
  if (optind < argc) {
    ifname = argv[optind];
  }
  ...
  if ((fname == NULL) && (ifname == NULL)) {
    printf(
        "pcapsipdump version %s\n"
        "Usage: pcapsipdump [-fpU] [-i <interface>] [-r <file>] [-d <working "
        "directory>] [-v level] [-R filter]\n"
        " -f   Do not fork or detach from controlling terminal.\n"
        " -p   Do not put the interface into promiscuous mode.\n"
        " -R   RTP filter. Possible values: 'rtp+rtcp' (default), 'rtp', "
        "'rtpevent', 't38', or 'none'.\n"
        " -U   Make .pcap files writing 'packet-buffered' - slower method,\n"
        "      but you can use partitially written file anytime, it will be "
        "consistent.\n"
        " -v   Set verbosity level (higher is more verbose).\n"
        " -n   Number-filter. Only calls to/from specified number will be "
        "recorded\n"
#ifdef USE_REGEXP
        "      Argument is regular expression. See 'man 7 regex' for details\n"
#endif
        " -t   T.38-filter. Only calls, containing T.38 payload indicated in "
        "SDP will be recorded\n",
        PCAPSIPDUMP_VERSION);
    return 1;
  }
  ...
  while ((res = pcap_next_ex(handle, &pkt_header, &pkt_data)) != -2) {
    {
      ...
      if (header_ip->protocol == 17) { // UPPROTO_UDP=17
        ...
        datalen =
            pkt_header->len - ((unsigned long)data - (unsigned long)pkt_data);
        ...
        if (opt_rtpsave == RTPSAVE_RTP) {
          ...
          is_rtcp =
              (htons(header_udp->source) & 1) && (htons(header_udp->dest) & 1);
        } else if (opt_rtpsave == RTPSAVE_RTPEVENT && datalen == 18 &&
                   (data[0] & 0xff) == 0x80 && (data[1] & 0x7d) == 0x65) {
          save_this_rtp_packet = 1;
        } else {
        ...
        if (save_this_rtp_packet &&
            ct->find_ip_port_ssrc(
                ...
                get_ssrc(data, is_rtcp), &idx_leg, &idx_rtp)) {
          if (ct->table[idx_leg].f_pcap != NULL) {
            ct->table[idx_leg].last_packet_time = pkt_header->ts.tv_sec;
            pcap_dump((u_char *)ct->table[idx_leg].f_pcap, pkt_header,
                      pkt_data);
            if (opt_packetbuffered) {
              pcap_dump_flush(ct->table[idx_leg].f_pcap);
                   ...
                   ct->find_ip_port_ssrc(
                       header_ip->saddr,
                       htons(header_udp->source) & rtp_port_mask,
                       get_ssrc(data, is_rtcp), &idx_leg, &idx_rtp)) {
          if (ct->table[idx_leg].f_pcap != NULL) {
            ct->table[idx_leg].last_packet_time = pkt_header->ts.tv_sec;
            pcap_dump((u_char *)ct->table[idx_leg].f_pcap, pkt_header,
...
#else
          if (number_filter[0] == 0 || (strcmp(number_filter, caller) == 0) ||
              (strcmp(number_filter, called) == 0)) {
            number_filter_matched = true;
          }
#endif
          if (s != NULL && ((idx = ct->find_by_call_id(s, l)) < 0) &&
            ...
            if ((idx = ct->add(s, l, pkt_header->ts.tv_sec)) < 0) {
              printf("Too many simultaneous calls. Ran out of call table "
                     "space!\n");
            } else {
              if ((strcmp(sip_method, "INVITE") == 0) ||
                  (strcmp(sip_method, "OPTIONS") == 0) ||
                  (strcmp(sip_method, "REGISTER") == 0)) {
                struct tm *t;
                t = localtime(&pkt_header->ts.tv_sec);
                sprintf(str2, "%04d%02d%02d", t->tm_year + 1900, t->tm_mon + 1,
                        t->tm_mday);
                mkdir(str2, 0700);
                sprintf(str2, "%04d%02d%02d/%02d", t->tm_year + 1900,
                        t->tm_mon + 1, t->tm_mday, t->tm_hour);
                mkdir(str2, 0700);
                sprintf(
                    str2, "%04d%02d%02d/%02d/%04d%02d%02d-%02d%02d%02d-%s-%s",
                    t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour,
                    t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour,
                    t->tm_min, t->tm_sec, caller, called);
                memcpy(str1, s, l);
                str1[l] = '\0';
                strcat(str2, "-");
                strcat(str2, str1);
                ...
                *strstr(str2, ".raw") = '\0';
                ...
                strncpy(ct->table[idx].fn_pcap, str2,
                        sizeof(ct->table[idx].fn_pcap));
              } else {
                if (verbosity >= 2) {
          ...
          if (idx >= 0 && l > 0 && strncasecmp(s, "application/sdp", l) == 0 &&
              strstr(data, "\r\n\r\n") != NULL) {
            in_addr_t tmp_addr;
          ...
          if (verbosity >= 3) {
            ...
            printf("Skipping udp packet %s:%d->%s:%d\n", st1,
                   htons(header_udp->source), st2, htons(header_udp->dest));
          }
        }
      }
    }
...
int get_sip_peername(char *data, int data_len, const char *tag, char *peername,
                     int peername_len) {
  unsigned long r, r2, peername_tag_len;
  char *peername_tag = gettag(data, data_len, tag, &peername_tag_len);
  if ((r = (unsigned long)memmem(peername_tag, peername_tag_len, "sip:", 4)) ==
      0) {
    goto fail_exit;
  }
  r += 4;
  if ((r2 = (unsigned long)memmem(peername_tag, peername_tag_len, "@", 1)) ==
      0) {
    goto fail_exit;
  }
  if (r2 <= r) {
...
int get_ip_port_from_sdp(char *sdp_text, in_addr_t *addr,
                         unsigned short *port) {
  unsigned long l;
  char *s;
  char s1[20];
